            ...
            <ul className="flex space-x-6">
              {menuItems
                ...
                .map((item) => (
                  <li
                    key={item.id}
                    className={item.type === "dropdown" ? "group mt-[3px]" : ""}
                    onMouseEnter={
                  ...
                  >
                    {item.type === "link" ? (
                      <Link
                        to={item.link}
                        className="text-white hover:text-gray-200 font-medium uppercase text-sm"
                      >
                        {item.label}
                      </Link>
                    ) : (
                            ...
                            <div className="flex">
                              <div className="w-2/3 p-8 grid grid-cols-2 gap-8">
                                {item.data.main.map((section, idx) => (
                                  <div key={idx}>
                                    <h3 className="font-bold text-[#1e3a5f] mb-4 uppercase text-sm tracking-wide">
                                      {section.title}
                                    </h3>
                                    <div className="space-y-2">
                                      {section.links.map((link, lIdx) => (
                                        <Link
                                          ...
                                          className="block bg-[#e0e0e0] hover:bg-[#2c4a6e] hover:text-white px-4 py-3 transition-colors text-sm text-[#1e3a5f]"
                                        >
                                          {link.label}
                                        </Link>
                                      ))}
                              ...
                              <div className="w-1/3 bg-[#45cbb2] p-8 flex flex-col justify-center space-y-4">
                                {item.data.side.map((link, idx) => (
                                  <Link
                                    ...
                                    to={link.to}
                                    onClick={handleLinkClick}
                                    className="block bg-[#1e3a5f] hover:bg-[#152943] text-white px-4 py-3 transition-colors shadow-md group/btn"
                                  >
                            ...
                            <div className="p-8">
                              <div className="grid grid-cols-3 gap-6">
                                {item.data.map((journal, index) => (
                                  <div
                                    key={index}
                                    className="bg-[#e0e0e0] p-4 rounded shadow-sm hover:shadow-md transition-all group/journal hover:bg-[#2c4a6e]"
                                  >
                                    <Link
                                      to={journal.link}
                                      onClick={handleLinkClick}
                                    ...
                                    >
                                      <div className="flex items-start gap-3">
                                        <span className="text-xl text-gray-700 mt-1 group-hover/journal:text-white transition-colors">
                                          {journal.icon}
                                        </span>
                                        <div>
                                          <strong className="text-[#204066] text-sm block mb-1 group-hover/journal:text-white transition-colors">
                                            {journal.title}
                                          </strong>
                                          <div className="text-xs text-gray-600 group-hover/journal:text-gray-200 transition-colors">
                              ...
                              <div className="grid grid-cols-3 gap-4">
                                {item.data.map((link, idx) => (
                                  <Link
                                    ...
                                    to={link.to}
                                    onClick={handleLinkClick}
                                    className="block bg-gray-200 hover:bg-[#2c4a6e] px-6 py-4 rounded transition-colors text-center group/author"
                                  >
                                    <span className="text-gray-800 text-sm font-medium group-hover/author:text-white transition-colors">
                                      {link.label}
                                    </span>
                                  </Link>
                                ))}
                            ...
                            <div className="flex w-full">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                  {item.data.links.map((link, idx) => (
                                    <Link
                                      ...
                                      to={link.to}
                                      onClick={handleLinkClick}
                                      className="block bg-[#e0e0e0] hover:bg-[#2c4a6e] hover:text-white px-4 py-4 transition-colors text-sm text-[#1e3a5f] text-center font-medium"
                                    >
                                      {link.label}
                                    </Link>
                                  ))}
                              ...
                              <div className="w-1/3 bg-[#45cbb2] p-8 flex flex-col justify-center items-center">
                                <Link
                                  to={item.data.cta.to}
                                  onClick={handleLinkClick}
                                  className="w-full bg-[#1e3a5f] hover:bg-[#152943] text-white px-6 py-4 transition-colors shadow-md group/btn flex items-center justify-between uppercase text-sm font-bold tracking-wider"
                                >
                                  {item.data.cta.label}
                ...
                <li key={item.id}>
                  {item.type === "link" ? (
                    <Link
                      to={item.link}
                      onClick={handleLinkClick}
                      className="block px-4 py-2 text-white hover:bg-[#3a5a8e]"
                    >
                      {item.label}
                    </Link>
                  ) : (
                    <>
                      ...
                      >
                        {item.label}
                        <FaChevronDown
                          className={`text-xs transition-transform ${activeDropdown === item.id ? "rotate-180" : ""}`}
                        />
                      </button>
                      ...
                      {activeDropdown === item.id && (
                        <div className="bg-[#3a5a8e] px-6 py-2 space-y-2 max-h-64 overflow-y-auto">
                          ...
                          {item.layout === "journals" &&
                            item.data.map((journal, idx) => (
                              <Link
                                ...
                                to={journal.link}
                                onClick={handleLinkClick}
                                className="block py-1 text-gray-200 hover:text-white text-sm"
                              >
                                {journal.title}
                              </Link>
                            ))}
                          ...
                          {item.layout === "conference" && (
                            <div className="space-y-2">
                              <div className="text-xs font-bold text-gray-400 uppercase tracking-widest pt-2">
                                Conferences
                              </div>
                              {item.data.links.map((link, idx) => (
                                <Link
                                  ...
                                  to={link.to}
                                  onClick={handleLinkClick}
                                  className="block py-1 text-gray-200 hover:text-white pl-2 border-l-2 border-transparent hover:border-[#45cbb2] transition-colors"
                                >
                                  {link.label}
                                </Link>
                              ))}
                              <div className="pt-2">
                                <Link
                                  to={item.data.cta.to}
                                  onClick={handleLinkClick}
                                  className="block bg-[#45cbb2] text-[#1e3a5f] py-2 px-4 text-center font-bold uppercase text-sm hover:bg-[#34a892] transition-colors"
                                >
                                  {item.data.cta.label}
                                </Link>
                              </div>
